function x_opt = levenberg_marquardt(x_ini, y_targets, fun, dfun, x_min, x_max, max_iters, tolerance, delta_trust, nu_val)
...
% Levenberg-Marquardt algorithm main loop
    ...
    iters = 1;
    x_opt = x_ini;
    residuals = y_targets.flatten() - feval(fun, x_ini).flatten()
    ...
    while iters <= max_iters && ~exit_flag
        ...
        for sub_iters = 1:10            
            ...
            residuals_test = y_targets - feval(fun, x_test);
        ...
        loop_time = toc(loop_init_time);
        elapsed_time = toc(init_time);
        display(['Iteration ', num2str(iters), ', Loss: ', num2str(Loss), ', ||Jr||: ', num2str(norm(Jr)), ', loop time: ', num2str(loop_time), ', elapset time: ', num2str(elapsed_time)]);
        iters = iters + 1;
    end
end
...
% norm lower or equal to delta_trust
    ...
    lambda_search_limits = [-min(diag(L))+1e-16, 10*max(diag(L))];
